...
# red
log_error() { color_text "[ERROR] $(date '+%Y-%m-%d %H:%M:%S') - $1" "31"; }
# green
log_info() { color_text "[INFO] $(date '+%Y-%m-%d %H:%M:%S') - $1" "32"; }
# yellow
log_warn() { color_text "[WARN] $(date '+%Y-%m-%d %H:%M:%S') - $1" "33"; }
...
# mysql 8.0.46 glibc2.28包下载地址："https://cdn.mysql.com/Downloads/MySQL-8.0/mysql-8.0.46-linux-glibc2.28-x86_64.tar.xz"
# mysql 8.0.46 glibc2.17包下载地址："https://cdn.mysql.com/Downloads/MySQL-8.0/mysql-8.0.46-linux-glibc2.17-x86_64.tar.xz"
# mysql 8.4.10 glibc2.28包下载地址："https://cdn.mysql.com/Downloads/MySQL-8.4/mysql-8.4.10-linux-glibc2.28-x86_64.tar.xz"
# mysql 8.4.10 glibc2.17包下载地址："https://cdn.mysql.com/Downloads/MySQL-8.4/mysql-8.4.10-linux-glibc2.17-x86_64.tar.xz"
...
while getopts 'a:v:t:h' opt; do
  ...
  h)
    log_info "Usage: $0 [-a install|uninstall] [-v <mysql_version>] [-t <binary|source|repository>]"
    exit 0
    ;;
  *)
    log_error "Usage: $0 [-a install|uninstall] [-v <mysql_version>] [-t <binary|source|repository>]"
    exit 1
    ;;
  esac
done
...
*)
  log_error "无效的操作: ${ACTION}，仅支持 install 或 uninstall"
  exit 1
  ;;
esac
