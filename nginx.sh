...
while getopts 'a:v:t:h' opt; do
  ...
  h)
    log_info "Usage: $0 [-a install|uninstall] [-v <nginx_version>] [-t <source|repository>]"
    exit 0
    ;;
  *)
    log_error "Usage: $0 [-a install|uninstall] [-v <nginx_version>] [-t <source|repository>]"
    exit 1
    ;;
  esac
done
...
install_nginx_source() {
  ...
  for packages in make gcc gcc-c++ openssl-devel zlib-devel gd-devel; do yum -y install $packages; done
