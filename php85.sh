...
while getopts 'a:v:t:h' opt; do
  ...
  h)
    log_info "Usage: $0 [-a install|uninstall] [-v <php_version>] [-t <source|repository>]"
    exit 0
    ;;
  *)
    log_error "Usage: $0 [-a install|uninstall] [-v <php_version>] [-t <source|repository>]"
    exit 1
    ;;
  esac
done
